...
const TasksLeftSide = () => {
  ...
  const updateCheckStatus = (index: number) => {
    setBadges(
      badges.map((badge, currentIndex) =>
        currentIndex === index ? { ...badge, checked: !badge.checked } : badge
      )
    );
  };
  ...
  const dispatchAddFunction = (
    ...
    if (title && comment.length < 50) {
      ...
      buttonPushTL.to(currentTarget, {
        duration: 0.25,
        y: 2.5,
        boxShadow: "4px 2.5px 0px 0px #023E7D",
      });
      buttonPushTL.to(currentTarget, {
        duration: 0.25,
        y: 0,
      ...
      linePushTL.to(horizontalLineRef.current, {
        duration: 0.25,
        backgroundImage: `linear-gradient(to Right, ${
          addedBoolean ? "#ABC4FF, #0466C8" : "#ABC4FF, #EC214E"
        }`,
        ease: "linear",
      });
  ...
  const callbackForDispatchAdd = () => {
    dispatch(
      addActiveTodo({
        ...
        title: title,
        badges: badges.filter((item) => item.checked),
        comment: comment,
      ...
      <div className="row d-flex justify-content-center align-items-start rounded">
        <div className="col-lg-12 row text-center d-flex justify-content-center">
          ...
          <div className="col-lg-12 text-center">
            <i>maximum of 50 characters for comments</i>
          </div>
          <input
            className="p-1 m-1 col-lg-5"
            type="text"
